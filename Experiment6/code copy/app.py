...
def swich_choice_tran_article(choice, title, link, content, tran_content):
    # 携带参数的函数对象
    print_article_with_default = functools.partial(back_to_content, title, link, content)
...
def back_to_content(title, link, content):
    new_choice = print_article(title, link, content)
